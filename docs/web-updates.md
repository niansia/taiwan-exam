...
## 2026.09.22.25：修正國寫「字體角色」檢查的誤報

一份以 2026.09.22.24 產生的國寫卷，其他檢查都通過，只有最終檢查的「字體角色」回報題本說明框與閱讀材料「應該用明體」。實際上說明框與材料是楷體、題目是明體，和官方一致，是檢查程式的錯：它只讀「完全落在寫死版心 64～531 pt 內」的文字行，而說明框第一行右緣在 536.8 pt、「一、」左緣在 63.9 pt，這兩行被略過，它們決定的字體角色沒被讀到，底下正確的楷體行就全被判錯。每份國寫卷都有同一段官方說明，所以每份都會出現，改題目也修不掉。

- 檢查改成只以高度排除頁首頁尾，不再要求文字行左右完全落在版心內。
- 說明框改用頁面上畫出的框線辨認，不再只靠「說明：」開頭；「（一）」「（二）」題號行、第二大題寫作任務（「請以『…』為題」「（占25分）」）都會切回明體判斷。第二大題這一點是修第一個錯時發現的：若不一起修，每份國寫卷的第二大題寫作任務都會被誤判。
- 全科檢查同類問題：最終檢查中會直接判不通過、又依賴寫死版心的只有這一項；其他用到版心的檢查在範圍外只會略過、不會誤判。七科版型範例全部重跑最終 PDF 檢查，都沒有會擋下的項目。
...
- **9 月 24 日第二次替換 ZIP（版本號不變）**，兩個排版小錯：
  - **頁碼超過 9 頁**：頁首頁尾的頁碼欄位是照一位數頁碼量的，「10」比頁尾欄位寬 0.40 pt，七科都一樣，所以內頁一到第 10 頁就被拒絕（官方 115 國綜就有 11 頁內頁）。一次國綜執行中，模型自行放寬了這個限制才做出詳解。現在頁碼欄位可以超出最多 1 pt，模板 PDF 不動，最終檢查仍逐頁比對鎖定像素；標題欄位維持原本的嚴格寬度。模板上頁尾的兩個「-」是照一位數位置鎖定的，所以第 10 頁起印成「-10-」，比官方「- 10 -」略緊。
...
- **9 月 24 日第四次替換 ZIP（版本號不變）**：數A 選項排版。一份數A 把 13～15 字的選項排成三個一列（`grid-3-2`），選項在欄內換行，「相等」的「等」和分數 4/5 都掉到下一行。官方 111～115 數學卷的選項只有兩種排法：短選項五個一列，其餘一行一個（三加二只出現一次，而且都是很短的選項）。現在數A／數B 的選項若放不進所選欄位的官方欄寬，就改成一行一個，不再在欄內換行；短選項照原本選的排法。新增測試（舊程式失敗、新程式通過，PyMuPDF 1.26 亦同）。
- **9 月 25 日第五次替換 ZIP（版本號不變）**：圖表標籤用中文。一份自然卷的表 1 欄名是「sample／sulfate／carbonate」，圖上標「electrolyte」「reaction progress」，官方會寫「樣品」「硫酸根」「電解液」「反應進程」。現在英文以外的科目，圖表上出現英文單字（四個字母以上、非全大寫）就會被擋下；符號、單位、化學式和縮寫（x、t (s)、mol、NaCl、DNA、NOAA、LED）照常可用，真實照片裡的招牌文字不算。檢查會讀圖表的 `semantic_data` 標籤、SVG 文字與 PDF 圖檔，畫完圖執行 `check-figures` 時就會回報，最終檢查也會再擋一次；PNG 點陣圖讀不到字，說明文件已要求逐頁檢查時確認。新增測試。
- **9 月 25 日第六次替換 ZIP（版本號不變）**：各科難度都有可接受範圍，並可自訂難度。
...
- **9 月 26 日第八次替換 ZIP（版本號不變）**，兩個小錯：
  - **社會考試說明的雜湊值**：社會科的命題範圍檢查要求每份卷記錄大考中心〈社會考科考試說明〉與國教院社會領綱 PDF 的網址、取得日期與 SHA-256，模型每次都得連回官網下載；大考中心拒絕連線時整份卷就卡住。這兩份文件的內容 Skill 早已整理內建（必修學習內容代碼、評量目標 H/G/C/S），現在由維護者下載一次、把網址、大小、SHA-256 與四個章節（測驗目標、測驗內容、題型配分、試題舉例）的頁碼和摘要固定在 `references/social-scope-sources.json`（PDF 不收進專案）。模型沒填網址與雜湊時檢查自動採用這份紀錄，仍須記錄讀過四個章節；若模型實際下載到雜湊不同的版本，代表官方改版，需記錄並告知使用者。
...
- **9 月 26 日第十次替換 ZIP（版本號不變）**：社會與自然的照片要求說明不一致。一份社會卷在環境無法下載照片時，要求使用者上傳「至少四張附來源與使用授權資訊的真實照片」；但檢查程式的下限是 2 張，而且來源可追溯即可、不需授權（9 月 24 日已改）。原因是模型開始出卷時讀到的「本科必做清單」和社會科說明還寫著舊的「四張照片」，SKILL.md 也還有「照片須原創、公有領域或授權」的舊句子。自然科清單同樣還寫著舊的「3 張照片」（下限是 1 張）。現在清單改成現行規定：社會一年內時事 10 題（180 天內 4 題）、圖表至少 10 項 4 種、真實照片至少 2 張、18 題引用圖表；自然真實照片至少 1 張；兩科都寫明先上網找、找不到用照片庫、來源可追溯即可不需授權。社會科說明也寫明連不上網時照片庫 2 張就夠、不要向使用者要更多。新增測試把清單數字綁到程式實際的下限，以後改下限卻漏改清單會直接失敗。
- **9 月 26 日第十一次替換 ZIP（版本號不變）**：表格圖的文字超出格線。一份社會卷的表 1 由模型畫成圖片，表頭「生態最低量（萬噸）」超出表格右邊框；七科的表格圖都可能發生。模型畫的圖多半是 PNG，讀不到文字，所以改從像素判斷：找出表格的橫線與直線，若某條格線在同一列兩側都緊貼著筆畫，就代表文字壓線或超出表格。只檢查表格（圖的種類為表格，或圖說為「表 N」），照片不檢查。畫完圖執行 `check-figures` 時就會回報，最終檢查也會再擋一次；畫圖說明也加上「表格文字須在格線內，過長就加寬欄位或把表頭拆成兩行，不得把字縮到小於內文」。以維護者回報的那張表驗證能抓到，已驗收考卷裡的表格都不會誤判；新增測試，PyMuPDF 1.26 亦驗證。

目前的 ZIP 是 9 月 26 日第十一次替換後的版本（SHA-256 `06b415f8ff9895dab09fe24f619e02c7ed6d89757035104f076fbd24847abb62`，8,672,131 位元組，101 個檔案），與解壓內容通過 Windows Defender 與 Windows 附件檢查；維護者已確認 Chrome 下載正常，並在 Claude 與 ChatGPT 上傳成功。前十一份 ZIP（`cac5344f…df74e3a`、`abd409e7…8f017b30`、`140b293d…c36a5c3c`、`c71da2aa…4a5909d2`、`e808397c…1455ad2`、`764c0db8…10b2c74`、`f65b85fb…c78632b`、`1a4e0ff8…78fa9b1c`、`911d8f26…92781ad4c`、`fd44241b…e70286284`、`be0eded1…52dc005f`）也曾分別確認。[下載 2026.09.22.25 ZIP](https://github.com/niansia/taiwan-exam/releases/download/hosted-2026.09.22.25/taiwan-exam-hosted-2026.09.22.25.zip)；security.json 與 browser.json 在 [Release](https://github.com/niansia/taiwan-exam/releases/tag/hosted-2026.09.22.25) 頁面。已安裝的舊 Skill 請重新下載替換。

## 2026.09.22.24：自然科圖表編號、選項、小題、時事來源與照片依官方 111～115

維護者交來 Claude（N116M1）與 ChatGPT 兩份 116 自然（以 2026.09.22.16 前後的舊版產生，字型仍是 Noto／霞鶩文楷，這部分已在 .17 起修正），和官方 111～115 比對。這版修正：

- **諾貝爾獎與同一張星空圖**：確實是 Skill 造成的。時事說明原本寫「自然請安排秋季諾貝爾獎」，檢查程式的錯誤訊息也拿諾貝爾獎當例子，兩份卷因此各出了三組諾貝爾題（Claude 卷物理、生醫、化學）。現在改成可自由選用期刊論文、國際會議、Nature／Science 報導、機關資料發布、監測序列（聖嬰、二氧化碳）、太空任務、災害報告、臺灣在地資料等；檢查程式要求一年內時事**諾貝爾最多 1 組**、**同一發布單位最多 2 個**、**至少 3 種來源家族**、**同一事件只能給一個題組**（ChatGPT 卷把同一次日食與同一則 Swift 公告各用了兩次，六個來源有三個是 NASA）。新增「生成卷已用過的素材」清單，哈伯極深空影像再出現即退件。
- **圖表編號**：官方每年標 16～28 個「圖 N」與 4～12 個「表 N」，題幹寫「如圖 3」；兩份卷都只寫「如圖」「如表」，因為排版程式不會印圖說。現在圖可設定 `caption`「圖 1」，印在圖下方置中；自然與社會科都要求每張圖表編號、依序、並在題目引用。
- **選項**：官方正解是唯一最長選項的單選題每年 1～3 題（7～14%），Claude 卷 22%、GPT 卷 52%；同題選項字數差中位數官方 1～5 字，兩卷 11～12 字。新增 `validate_natural_layout_contract.py`：最長正解至多 4 題、字數差中位數至多 6 字、選項不加「。」。選項排法改依官方：7 字內五個一列，其餘一行一個（原本會把 8～52 字的選項排成兩欄）。
- **命題性質**：GPT 卷幾乎每組都印「教學模型」「非 NASA 數據」，許多正解是「仍需另查資料」，還有「pH 約 300」這類荒謬選項。現在不得印這類說明，正解屬「再查證資料」者至多 3 題（官方每年 0～1 個）；審題須記錄預估答對率，選擇題平均不得高於 0.65（官方 0.53～0.57）。
- **小題**：官方 112～115 題號只印一次，小題用 (a)(b)；GPT 卷印「38(a)」「41(1)」，Claude 卷用 (1)(2)。現在 (a)(b) 懸掛對齊、詳解印「第 42 題(a)」，其他寫法會被擋。
- **照片**：官方自然卷以線條圖、曲線圖、表格為主，真實照片每年約 0～2 張，原本「至少 3 張照片」改為至少 1 張；圖表至少 16 項不變。
...
維護者交來 Claude（S116A）與 ChatGPT 兩份 116 社會（以 2026.09.22.16 產生），和官方 111～115 逐頁比對。字型與頁首已在 .17～.22 修正；這版修正其餘問題：

- **選項與正解（官方量測）**：官方每題四個選項約八成字數完全相同（相差兩字內者 89～96%），正解是唯一最長選項的每年只有 0～3 題；Claude 卷 54 題中 42 題、ChatGPT 卷 40 題的正解是最長選項，只挑最長的就能拿七成以上分數，這是兩份卷偏易、與學測差距大的主因。新增 `validate_social_layout_contract.py`：同題選項須寫成等長（至少 80% 題目相差兩字內）、正解最長者至多 4 題、選項句末不加「。」。審題須記錄每題預估答對率，選擇題平均不得高於 0.65（官方 0.51～0.60）。
- **題目性質**：ChatGPT 卷幾乎每題都是「虛構小鎮＋研究者應再蒐集哪些資料」，材料印出「教學虛構情境」「未抄錄館藏文案」，同一組戶數、就業、馬歇爾計畫、鐵路與博覽會資料在兩三個題組重複出現；官方五年沒有一題正解是「再蒐集資料」。現在正解屬此類者至多 3 題、不得印出虛構／教學情境等說明、同一組數據不得出現在兩份材料；說明文件要求每題都要用到一個必修概念。Claude 卷選材大致在課綱內。
- **版面**：題組標記改為官方「26-27 為題組」加底線（原印「第 26 至 27 題為題組」），材料每段首行縮排 24pt；選項依官方字數分列（七字內四個一列、十六字內兩個一列、其餘一行一個），取消原本「一律一行一個」的錯誤規定。契約要求大標「第壹部分、選擇題（占76分）」「第貳部分、混合題或非選擇題（占68分）」，非選配分寫成「（3 分）」或「（3 分，35 字內）」，不得寫「（本題3分）」「（10字以內）（3分）」，不印（1）（2）小題（官方以同一題的勾選欄與理由欄表格處理），不印「作答區／請依題意書寫／答＿＿」空白框。社會版型範例骨架改成官方標題與格式。
- **時事與跨科**：一年內時事題由 6 題提高到 10 題（半年內 4 題），須分散在至少 5 份材料、兩部分各至少 4 題；材料寫出年月。新增三科合一題組：第壹部分至少 1 組、第貳部分至少 2 組，組內各題分別以歷史、地理、公民為主科（115 年 30-33、36-38 題）。修正說明文件「3～5 題」與「6 題」互相矛盾的舊寫法。
- **照片**：取消「至少 4 張真實照片」；依官方每年 2～4 張照片或史料圖像、7～13 張圖表地圖、18～45 題用到圖表，改為至少 2 張照片、10 項圖表、18 題引用圖表。來源不再要求授權，網路找到、來源可追溯即可（記錄網址、網站或作者、取得日期、原檔與雜湊）。預檢會先測網路能否抓圖：能就先上網搜尋；不能（Claude 的環境只連得到 GitHub）就用新的照片庫：`photo_library.py` 從 [photos-v1 Release](https://github.com/niansia/taiwan-exam/releases/tag/photos-v1) 下載 59 張 Wikimedia Commons 灰階照片（地理 26、歷史 25、公民 8，公有領域／CC），附可觀察特徵與可連結課綱，裁切後自動產生來源紀錄。卷內設定只能提高圖片下限，不能降低。
...
維護者交來 Claude（CN116A）與 ChatGPT（g116z0923）兩份 116 國綜（以 2026.09.22.16 產生），和官方 111～115 逐頁比對。兩份選擇題答案都正確（Claude 卷第 26 題 C 可議），但估計平均答對率約 0.70 與 0.80，官方五年為 0.48～0.58。字型（全字庫正宋體／正楷體取代 Noto 與霞鶩文楷及其「説」「内」異體字）、頁首、選項固定欄距、大標粗體、詳解本簽名提醒已在 2026.09.22.17～21 修正；其餘這版修正：

- **混合題小題**：官方每題只印一次「32. …請回答下列問題：」，下面接（1）（2），各以「（占N分，作答字數：…）」作結，①② 寫在小題文字裡。原本排版程式沒有小題功能：ChatGPT 卷印成「32.（1）」「（2）①」「（2）②」，標籤位置三種、（2）重複出現；Claude 卷因同一標籤不能重複，改了三輪。現在每個（1）（2）存一筆（第一筆附 `number_stem` 引導句，題幹開頭寫「（1）」也可自動辨識），題號只印一次，小題懸掛對齊（標籤 x 82.1、續行 112.1，與 115 相同），詳解印「第32題（1）」。契約要求小題以官方格式標配分與字數、混合題單選以「（占2分，單選題）」作結。
- **題組與材料**：題組標記改為官方「6-8為題組。閱讀下文，回答6-8題。」；閱讀材料改用楷體、內縮 18pt、段首再縮 24pt、「甲、」段懸掛；單題所附材料印在題幹之後、選項之前（官方 115 第 27 題），不再印在題號上方；兩欄選項第二欄改在官方的 x 303。版型範例的國綜骨架也改成官方標題與混合題格式。
- **命題下限（官方量測）**：選擇題題幹以「：」作結，不用「？」；圓圈數字用 ①②，不用 ➀➁（存題即擋）；「」為第一層引號；出處不寫命題說明（命題者、假設情境、節寫）；正解是唯一最長選項者至多 8 題（官方 111 年 2 題、115 年 5 題；Claude 卷 19 題）；非選評分只用 2／0 或 4／2／0；審題須為每題記錄預估答對率，全卷平均不得高於 0.62。
- **原創性**：新增官方 111～115 國綜的雜湊指紋（不含原文），材料或題幹與官方任一年連續 39 字以上相同即退件。Claude 卷第 30–31 題重用 114 年同一段〈晚遊六橋待月記〉、同一題位，現在會被擋下。
- **流程**（依兩份計時回報）：只有長篇純文字材料的題組題不再要求單獨截圖校樣，詳解純文字截圖改在整頁上審查（題目與詳解分開判斷是否需要截圖）；`lock-content` 會先跑一遍最終檢查的科目契約並列出 `contract_problems`，要求建置前修完；說明文件要求沒看到的圖片不得記為通過、題組先試排分頁再校樣。
...
維護者交來 Claude（MB116X）與 ChatGPT（B260923）兩份 116 數B（以 2026.09.22.16 產生），和官方 111～115 逐頁比對。兩份 20 題答案都正確；Claude 卷估計平均答對率約 0.37（接近 115 的 0.37），ChatGPT 卷約 0.61（比最簡單的 111 年 0.50 還容易）。斜體、疊排分數、根號、選項欄距、問法、題組標記與頁首已在 2026.09.22.17～19 修正；其餘這版修正：

- **難度切點（維護者決定）**：數A、數B 的「中偏難＋難 ≥ 70 分、難 ≥ 30 分」原本沒有答對率切點，審題者各自判斷，同一題換人就換級。現在審題者必須為每題記錄預估答對率 `estimated_p`，難度級別由它決定：難 < 0.30、中偏難 0.30～0.50、中 0.50～0.70、簡單 ≥ 0.70。依此切點量官方數B 第 1～17 題，115 年正好是 70／30 分，所以門檻等於近五年最難的一年。級別與答對率不符，或「只需代公式」卻標中偏難，在存檔檢查點就會報出，不再等到最後。
- **參考公式頁**：原本以 Cambria Math 排版，改成和官方相同的 Times 斜體變數、上下疊排分數、有橫線的根號、細明體字距，每行基線與官方 115 相差不到 0.6pt；文字也改成官方寫法（「項之和為」、「R 為 ΔABC 外接圓半徑」、r 的下標 X,Y，標準差與相關係數補上第二項）。
- **封面**：第一行只印「Taiwan Exam」（Times），不再和第二行重複「模擬試題」；第二行改用楷體加 Times 數字；「—作答注意事項—」改官方破折號；數學卷的作答注意事項改為官方 12pt、行距與斷行（「恐將影響／成績。」），劃記例句加上答題卷小格圖示。
- **詳解本**：每頁不再印「請記得在答題卷簽名欄位以正楷簽全名」（考生提醒只屬於題本）。
- **算式排版**：全形「＝＋－＜＞」自動改為官方的半形 Times 符號；「|x − 2| < 3」「30p₁ + 60q₁ = 2700」這類算式不再從中斷行（Claude 詳解約 20 處）；選填答案格與前一個字、後面的「。」排在同一行（兩份卷第 13、14、17 題答案格掉到下一行）；題幹最後一行不再只剩「個？」「元？」；題號後的全形空白自動去除；「ab」這類變數乘積改斜體。
- **題間作答空間**：官方數學題本題與題之間留作答空間（題間中位數約 52pt），原本 12pt 題距讓空白全堆在頁底；現在每頁剩餘空間平均分到題與題之間（每處最多加 60pt），已超過空白上限的頁面不分配，仍會被擋。
- **題組表格**：題組文字寫「下表為…」時，表格或圖接在題組文字後、第 18 題前，不再夾在第 18 題下。
- **數B 單元與題型（官方量測）**：數據分析至多 2 題（Claude 卷 4 題）；多項式每年 2 題（Claude 卷 1 題）；歸為矩陣單元的題目必須出現矩陣（ChatGPT 卷只用文字描述線性變換）；選填 13–17 至少一題「（化為最簡分數）」（官方每年數A 1–4 題、數B 1–3 題；ChatGPT 卷五題全是兩位整數）；第 19、20 題詳解須附評分原則。
...
維護者交來 Claude（EN116M1）與 ChatGPT（E260923B）兩份 116 英文卷（以 2026.09.22.16 產生），和官方 111～115 逐頁比對。兩份 50 題答案都正確。下列問題中，有些在 2026.09.22.17～19 已修（第壹／第參部分標題、選項固定欄位、說明框楷體的「説／爲／内」異體字與「⾋」、頁首「116年英文」），其餘這版修正：

- **底線**：綜合測驗、文意選填、篇章結構的編號空格在 ChatGPT 環境（PyMuPDF 1.26.0）沒有底線，因為 1.26 不畫 `<u>` 標籤；改用 CSS 畫線，數字改 Times。
- **混合題**：官方 112～115 都是「47-48」一題（中文說明＋一句摘要句內含 47、48 兩格，每格填文中一個單詞並做字形變化，「（填充題，4分）」）、49「（多選題，4分）」不寫應選數量、50「（簡答題，2分）」答文中的片語，下方印一條作答線（113～115）。舊規則要求材料跨頁分段，題目因此被插在段落中間（Claude 卷 47、48 夾在第 3、4 段之間），圖表又跟著第 47 題印（ChatGPT 卷的表格夾在 47、48 之間）。現在材料全文先印、圖表接在材料最後、題目在後；契約改檢查上述題型、配分標示、單詞答案、50 的答案須出自文章，並拒絕「Choose TWO」。
- **中譯英與作文**：官方句首「1.」用 Times、句子用標楷體，不印配分；作文以「提示︰」開頭、楷體、續行懸掛，不印配分，也不再多一段「說明」。兩份卷都多印了「（4分）」「（20分）」，ChatGPT 卷還重複印了說明；已修正。英文題本題目內的中文（如 49 的中文作答指示）一律用楷體，題組標記維持明體。
- **選項**：文意選填選項庫依官方每 96pt 一欄、五欄排滿（1.26 忽略表格寬度，ChatGPT 卷只擠在左半邊）；綜合測驗的長片語選項放不下四欄時，照官方 115 第 17、20 題改排兩欄；篇章結構與閱讀選項換行後懸掛對齊。
- **Times 連字**：MuPDF 會把 fi、fl、ff 排成連字，複製搜尋時字會斷開；官方五年文字層都沒有連字，已拆開。
- **命題下限（官方量測）**：綜合測驗至少 4 題以片語或句構為選項（官方每年 5～7 題；ChatGPT 卷 0 題）；詞彙題正解須記錄詞性，至少三種、單一詞性不超過 5 題（ChatGPT 卷 10 題全是動詞）；文意選填選項庫每種詞形至少兩個（Claude 卷有 3 題只看詞性就能作答）；閱讀題正解是唯一最長選項者至多 4 題（ChatGPT 卷 7 題）；學生卷不得出現「invented data」或以題號指稱文章內容。
- **詳解評分**：中譯英依官方「每個錯誤扣 0.5 分，相同錯誤只扣一次」；作文依內容、組織、文法句構、字彙拼字整體給分，字數明顯不足、未分段各扣 1 分。
