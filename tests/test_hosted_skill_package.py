...
def test_wheel_bundling_is_opt_in(tmp_path):
    ...
    if not vendored:
        with pytest.raises(ValueError,match='vendor/wheels'):
            build('wheel-test',tmp_path/'without.zip',bundle_wheels=True)
        return
    report=build('wheel-test',tmp_path/'with.zip',bundle_wheels=True)
    assert report['bundled_wheels']==len(vendored)>=1
    with ZipFile(report['archive']) as zipped:
    ...
    assert len(wheels)==manifest['bundled_wheels']['count']==len(vendored)
    for row in wheels:
        ...
        assert 'taiwan-exam-generator/'+row['path'] in names
