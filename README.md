...
完整卷開始前，Skill 會要求網頁代理自行開啟當科五年份原卷並建立
跨年度校準紀錄；使用者不必逐份尋找或上傳。若帳號關閉網頁搜尋、
PDF 無法開啟或只能抽取文字而看不到頁面，代理必須標示未通過的校準
項目。個別大考中心連結逾時不再阻止出卷：Skill 會使用已由 111～115
正式卷建立、附來源雜湊與逐頁紀錄的內建校準資料，並把即時連線狀態另記
為部分可用或不可用，不得把網路逾時誤說成「沒有校準」。

七科 115 版型的 30 個固定 PDF 元件也已內建公開直連、大小與
SHA-256。網頁代理必須自行取得當科封面、奇偶頁首頁尾與數學公式頁，
...
Contents API／base64，解碼後核對大小與 SHA-256。取得 base64 本身就是可驗證
的 binary transport，不得再以「沒有 binary handoff」直接拒絕。
`github-pages.zip`、整個儲存庫 ZIP 或全模板 ZIP 都不是考卷成品，也不是模板
傳輸方式；Skill 不應下載或交付它們，只能取得當科逐檔 PDF。
