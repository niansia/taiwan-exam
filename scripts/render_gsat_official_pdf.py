...
def main(argv=None)->int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="gsat-official-") as d:
            d=Path(d);hp=d/"paper.html";pp=d/"paper.pdf";hp.write_text(prepare_html(html),encoding="utf-8")
            c=subprocess.run([str(browser),*browser_flags(d),"--no-pdf-header-footer",f"--print-to-pdf={pp}",hp.as_uri()],capture_output=True,text=True,timeout=90)
