...
.stimulus { margin:1mm 0 2mm; text-align:justify; white-space:pre-wrap; }
...
def render_question(q: dict[str,Any], base:Path|None, subject:str, *, show_stimulus:bool, show_visual:bool, group_range:str|None) -> str:
    page_break='<div class="page-break"></div>' if q.get("page_break_before") else ""
    label=f'<div class="group-label">{esc(group_range)}</div>' if group_range else ""
    stimulus=f'<div class="stimulus">{text_block(q.get("group_stimulus"))}</div>' if show_stimulus and q.get("group_stimulus") else ""
    ...
    score=f'<span class="score">（{esc(q.get("score"))}分）</span>' if q.get("score") is not None else ""
    split_class=" allow-split" if q.get("allow_page_split") else ""
    return f'{page_break}{label}{stimulus}<article class="question{split_class}"><div class="qno">{esc(q["number"])}.</div><div><div class="prompt">{score}{text_block(q["prompt"])}</div>{visual}{options}{lines}</div></article>'
...
def render(exam:dict[str,Any], *, include_answers:bool=True, asset_base:Path|None=None, run_contract:Path|None=None) -> str:
    ...
    trace=(f'內部實測卷｜來源與版本登錄：{meta.get("paper_profile_id")}｜'
           '校準基準：111—115學測｜非大考中心正式試題')
    ...
    return f'''<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><title>{esc(meta["title"])}</title><style>{STYLE}\n{custom_css}</style></head><body><div class="running-head"><span>第　頁<br>共　頁</span><span class="center">請記得在答題卷簽名欄位以正楷簽名</span><span class="right">116年學測<br>{esc(short)}</span></div>{cover(meta,exam["instructions"])}<main class="paper">{"".join(sections)}{continuation}<div class="trace">{esc(trace)}</div></main>{answer_html}</body></html>'''
