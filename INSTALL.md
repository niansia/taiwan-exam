...
這份文件主要給 AI 代理讀。目標是讓使用者只需貼一次儲存庫網址，之後直接用自然語言出題；不要把終端機操作轉嫁給不會寫程式的使用者。給一般使用者複製的各平台快速開始文字放在 [README](README.md)，補充操作見[各平台使用指引](docs/usage-guide.md)；本文件維護安裝、驗證與安全備援細節。
...
Claude 原生 Skills 優先使用 2026.09.19.4 ZIP，不解壓直接上傳並啟用。ZIP 內建七科模板與版型，出卷時不必另附。
...
官方說明：<https://help.openai.com/en/articles/20001066-skills-in-chatgpt>、<https://learn.chatgpt.com/docs/skills-and-plugins>、<https://learn.chatgpt.com/docs/build-skills>
...
原生 Skill：到 **Customize → Skills → ＋ → Create skill → Upload a skill**，上傳 [2026.09.19.4 工具／Skill ZIP](https://github.com/niansia/taiwan-exam/releases/download/hosted-2026.09.19.4/taiwan-exam-hosted-2026.09.19.4.zip)，不要解壓，按 Save 並啟用。這個版本使用短入口與分開的規則／工具；舊 MD 雖可符合 YAML 格式，整份讀入仍會浪費上下文，請停用舊 Skill 後改用 ZIP。MD 留給聊天專案知識。Claude 的實際上傳與平台掃描結果仍以帳號介面為準。

**Cowork** 是輸入框可選的任務模式：使用已啟用的 Skill，附上當科版型後提出出卷需求；不是把 Claude Code 的本機安裝指令貼到一般聊天。**Claude for Word** 是 Word 外掛，本專案尚未驗證該外掛的固定模板 PDF 流程。初次設定見 [README 的 Claude 安裝](README.md#claude)，出卷文字見 [README 第 3 步](README.md#第-3-步出卷)。
